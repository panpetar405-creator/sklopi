...
1. Dodaj ili izmeni tekst u `sr.json` (ključ → srpski tekst).
...
Dodaj red u `languages.json`, npr.:

```json
{"code": "de", "short": "DE", "name": "Deutsch", "locale": "de-DE"}
```

i pokreni `node translate.mjs`. To je sve: prekidač jezika, datumi, množina i nazivi gradova preuzimaju se iz tog reda i iz `de.json`. (Opciono polje `"tone"` menja stil prevoda, `"dateMonth":"short"` skraćuje mesec u datumima.) Kad jezika bude više od 4–5, prekidač bi trebalo pretvoriti u padajući meni.

## Ručne ispravke

Slobodno menjaj `en.json`, `ru.json`, … Skripta **ne prepisuje** ručno ispravljen prevod. Ako posle toga promeniš srpski tekst, ključ se samo prijavi kao „proveri ručno". Ako želiš da ga ipak ponovo prevede: `--retranslate-changed` (ili `--retranslate kljuc1,kljuc2`).
...
| `node find-hardcoded.mjs` | izlista srpski tekst upisan direktno u kod |
...
- `_notes.json` — kontekst za prevodioca po ključu ili prefiksu (npr. „SEO naslov, do ~60 znakova"). `_glossary.json` — reči koje se ne prevode.
