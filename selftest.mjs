...
// 1) novi jezici: de (2 oblika množine), pl (4), ja (1)
const cfg = J('languages.json');
cfg.languages.push({ code: 'de', short: 'DE', name: 'Deutsch', locale: 'de-DE' },
...
const sr = J('sr.json'), de = J('de.json');
ok(Object.keys(de).length === Object.keys(sr).length, 'de.json ima sve ključeve iz sr.json');
ok(de['plural.night'].split('|').length === 2 && J('pl.json')['plural.night'].split('|').length === 4 && J('ja.json')['plural.night'].split('|').length === 1,
   'broj oblika množine prati jezik (de 2, pl 4, ja 1)');
ok(run('--check').status === 0, '--check prolazi');
ok(/\bconst I18N\b/.test(fs.readFileSync(path.join(tmp, 'i18n-data.js'), 'utf8')) && fs.readFileSync(path.join(tmp, 'i18n-data.js'), 'utf8').includes('"de"'), 'i18n-data.js sadrži nove jezike');

// 2) promena srpskog teksta
sr.hero_lede = sr.hero_lede + ' (izmena)';
W('sr.json', sr);
const deEdit = J('de.json'); deEdit.nav_how = 'RUČNO POPRAVLJEN'; W('de.json', deEdit);
sr.nav_how = sr.nav_how + ' (izmena)'; W('sr.json', sr);
r = run('--mock');
ok(J('de.json').hero_lede.includes('(izmena)'), 'automatski prevod se osvežava kad se srpski promeni');
ok(J('de.json').nav_how === 'RUČNO POPRAVLJEN', 'ručno ispravljen prevod se NE prepisuje');
...
ok(J('de.json').nav_how.includes('(izmena)'), '--retranslate-changed prepisuje i ručni prevod');

// 3) obrisan ključ
delete sr.hero_lede; W('sr.json', sr);
ok(/kojih nema/.test(run('--check').stdout), 'ključ kojeg više nema u sr.json se prijavljuje');
run('--mock', '--prune');
ok(!('hero_lede' in J('de.json')), '--prune briše višak');

// 4) nedostajući ključ se ponovo dopunjuje
const de2 = J('de.json'); delete de2.nav_dest; W('de.json', de2);
ok(run('--check').status === 1, '--check javlja grešku kad prevod fali');
run('--mock');
ok(!!J('de.json').nav_dest, 'ključ koji fali se dopunjuje');
