/* ==========================================================
   ...
   Cloudflare Worker koji:
     1) na cron raspored (podešeno u wrangler.toml) prolazi kroz sve
        price_alerts sa status='active', ponovo računa procenjenu cenu
        (pricing-core.js) i šalje mejl kad cena padne ispod praga;
     2) izlaže /unsubscribe?token=... rutu za "Odjavi me" link u mejlu
        (jer klijent iz browsera nema update pravo na price_alerts — vidi
        supabase/price_alerts.sql).

   ENV / SECRETS potrebni (postavi preko `wrangler secret put <ime>`):
     SUPABASE_URL              — isto što i window.SKOKNICA_SUPABASE_URL
     SUPABASE_SERVICE_ROLE_KEY — service_role ključ (NIKAD anon/public!
                                  ovaj ključ zaobilazi RLS, čuvaj ga
                                  samo kao Worker secret, nikad u
                                  frontend kodu)
     RESEND_API_KEY            — API ključ za slanje mejlova (resend.com)
     ALERT_FROM_EMAIL          — npr. "Skoknica <alerti@skoknica.rs>"
                                  (mora biti verifikovan domen kod Resend-a)
     SITE_URL                  — npr. "https://skoknica.rs" (za linkove u mejlu)
========================================================== */
...
async function handleUnsubscribe(url, env) {
  ...
  if (!res.ok) return new Response('Alert nije pronađen ili je već ugašen.', { status: 404 });
  ...
  return new Response(
    `<!DOCTYPE html><html lang="sr"><meta charset="UTF-8">
     <body style="font-family:sans-serif;max-width:480px;margin:60px auto;text-align:center;color:#16242A;">
       ...
       <p>Više nećeš dobijati obaveštenja za ovu pretragu. Možeš napraviti nov alert bilo kad na Skoknici.</p>
       <a href="${env.SITE_URL || 'https://skoknica.rs'}" style="color:#8f6423;">← Nazad na Skoknicu</a>
     </body></html>`,
...
async function checkAllAlerts(env) {
  ...
  if (!res.ok) {
    console.error('[price-alert-worker] neuspešno čitanje price_alerts:', await res.text());
    return;
  }
  ...
  for (const alert of alerts) {
    try {
      ...
      console.error('[price-alert-worker] greška za alert', alert.id, err);
    }
  }
}
...
async function checkOneAlert(alert, todayKey, env) {
  ...
  // Nije okinulo — samo ažuriraj poslednju viđenu cenu za istoriju/debug.
  await sbFetch(env, `price_alerts?id=eq.${alert.id}`, {
    method: 'PATCH',
    body: JSON.stringify({
      last_price: price,
...
async function sendAlertEmail(alert, price, env) {
  ...
  //   ili podesi rutu na skoknica.rs/go/unsubscribe da prosleđuje na Worker (Cloudflare "Route").

  const tierLabel = alert.selection && alert.selection.kind === 'search'
    ? (alert.selection.tierLabel || TIER_LABELS[alert.selection.tier] || '')
    : 'tvoj aranžman';

  const subject = `Cena za ${alert.dest} je pala na ${price}€`;
  const html = `
    <div style="font-family:'Work Sans',Arial,sans-serif;max-width:520px;margin:0 auto;color:#16242A;line-height:1.6;">
      <h2 style="font-family:Georgia,serif;color:#123138;">Skoknica 🔔</h2>
      <p>Procenjena cena za <b>${escapeHtml(alert.dest)}</b> (${tierLabel}) je pala na
        <b style="font-size:20px;color:#8f6423;">${price}€</b> — ispod tvog praga od ${alert.threshold}€.</p>
      <p style="color:#4B5D62;font-size:13.5px;">
        ⚠️ Ovo je i dalje ilustrativna procena, ne stvarna ponuda partnera — proveri tačnu cenu
...
/* Mala helper funkcija za Supabase REST (PostgREST) pozive sa service_role ključem. */
